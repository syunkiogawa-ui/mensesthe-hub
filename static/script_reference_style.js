...
async function loadFilterOptions() {
    try {
        ...
        if (!response.ok) {
            throw new Error('フィルターオプションの取得に失敗しました');
        }
        ...
        // プレイ内容オプション
        if (options.playContents) {
            options.playContents.forEach(content => {
                const option = document.createElement('option');
        ...
        if (options.appearances) {
            options.appearances.forEach(appearance => {
                const option = document.createElement('option');
...
async function loadTherapists() {
    try {
        ...
        displayTherapists();
        console.log(`${allTherapists.length}人のセラピストを取得しました`);
...
function showTherapistDetail(therapistId) {
    ...
    // モーダル内容の更新
    document.getElementById('modalTherapistName').textContent = `${therapist.name} (${therapist.age})`;
    document.getElementById('detailTherapistName').textContent = therapist.name;
    document.getElementById('detailTherapistAge').textContent = `${therapist.age}歳 (${therapist.age_group})`;
